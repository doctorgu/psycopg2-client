...
- name: _filter_by_key
  value:
    ...
    user_name: "            AND user_name ILIKE %(user_name)s"
...
- `#{item}` or `%(item)s`: Parameterized bind variable (mapped to `%(__f_...)s` in PostgreSQL).
...
> [!NOTE]
> The psycopg2 array feature is faster.
