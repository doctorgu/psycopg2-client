...
from psycopg2_client.query_by_key.query import Query
from psycopg2_client.query_by_key.query_util import (
    get_conditional,
    get_foreach,
    get_query_with_value,
)
from psycopg2_client.query_by_key.settings import Settings as QrySettings
...
def test_foreach_basic_in_clause():
    ...
    res = get_foreach(sql, params, dialect="psycopg2")
...
def test_foreach_mybatis_style_not_supported():
    ...
    with pytest.raises(ValueError, match="Invalid #foreach syntax"):
        get_foreach(sql, params, dialect="psycopg2")
...
def test_foreach_native_psycopg2_placeholder():
...
def test_foreach_missing_key_raises_key_error():
    ...
    with pytest.raises(KeyError, match="'missing_list' not in params"):
        get_foreach(sql, {}, dialect="psycopg2")
...
def test_foreach_none_value_raises_value_error():
    ...
    with pytest.raises(ValueError, match="'user_ids' value in params cannot be None"):
        get_foreach(sql, {"user_ids": None}, dialect="psycopg2")
...
def test_foreach_invalid_type_raises_type_error():
    ...
    with pytest.raises(
        ...
        get_foreach(sql, {"user_ids": 12345}, dialect="psycopg2")
...
def test_foreach_unclosed_raises_value_error():
    ...
    with pytest.raises(ValueError, match="Unclosed #foreach"):
        get_foreach(sql, {"user_ids": [1]}, dialect="psycopg2")
...
def test_foreach_missing_separator_defaults_to_empty():
    ...
    #endfor
    """
    result = get_foreach(sql, {"user_ids": ["a", "b"]}, dialect="psycopg2")
...
def test_foreach_nested_inside_if():
    ...
    step2_f = get_foreach(step1_f, params_f, dialect="psycopg2")
    ...
    step2_t = get_foreach(step1_t, params_t, dialect="psycopg2")
